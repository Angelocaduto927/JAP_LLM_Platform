...
def generate_prompt(knowledge_point: str, question_format: int, num_questions: int) -> str:
    ...
    # 两种出题模式：1=填入语法点；2=语法点已在题干中，需要据此完成另一处或续写
    interference_rules = {
        1: "The grammar point itself (or its correct conjugation) must fill the blank. The context should naturally require this grammar point, not just accept it.",
        2: "The grammar point appears in the stem. Use it to either complete another blank or choose the most natural continuation. Make the context require understanding/applying this grammar point."
    }

    # 三档难度
    base_count = max(1, int(num_questions * 0.4))
    intermediate_count = max(1, int(num_questions * 0.4))
    advanced_count = max(0, num_questions - base_count - intermediate_count)

    prompt = f"""
    You are an experienced Japanese examiner for JLPT N4/N5.
    Create exactly {num_questions} multiple-choice questions for the grammar point: {knowledge_point}.
    ...
    1) Unique question stems: no duplicates or near-duplicates across this set.
    2) Exactly one correct answer per question (no multiple reasonable answers under the given context).
    3) Options must be 4, mutually exclusive, and plausible.
    4) Keep strict output format:
    - Start with 'Qx: もんだい{question_format}'
    - Next line: the stem (must contain a single blank with empty parentheses: ( ))
    - Next line: '1. ... 2. ... 3. ... 4. ...'
    - Final line: 'Answer: x'
    - No extra text, comments, explanations, reasons, or metadata.
    5) Grammar/Vocab range: strictly N4/N5 level. Avoid keigo beyond N4, avoid domain-specific jargon.
    6) Kanji: Prefer N4/N5 level kanji. Provide kana where necessary to keep N4/N5 readability.
    7) Context authenticity: everyday scenes (shopping, school, schedule, email). Avoid sci-fi/professional topics.
    8) No hints in the stem that leak the answer (e.g., repeating the correct option verbatim).
    9) Parentheses ( ) for the blank must be present and empty in the stem.

    Soft constraints (quality enhancements):
    - Distractor design: three wrong options should reflect realistic learner errors for this grammar point, such as:
    a) Wrong conjugation or polarity
    b) Wrong particle or set phrase misuse
    c) Unnatural collocation or register mismatch
    - Answer-position balance: across the whole set, distribute correct answers over 1..4 positions (avoid long runs like 1,1,1).
    - Length: generally 1–2 sentences per stem; keep concise but sufficient to rule out distractors.
    - Natural style: polite or plain form consistent within each item.

    Examples (Good vs Bad):

    Good (Format 1: the grammar fills the blank)
    Q1: もんだい1
    市役所の手続きが終わった( )、家に帰ります。
    1. なら 2. たら 3. ので 4. ても
    Answer: 2

    Bad (two correct answers; avoid)
    Q2: もんだい1
    雨が( )、試合は中止です。
    1. 降れば 2. 降ったら 3. 降るなら 4. 降っても
    Answer: 1

    Good (Format 2: grammar appears in stem; apply it elsewhere or choose best continuation)
    Q3: もんだい2
    は条件を表す文法です。忘れ物( )、すぐに連絡してください。
    1. がしても 2. をしたら 3. がすれば 4. をすれば
    Answer: 4

    Output all {num_questions} questions strictly in this format, with no extra lines or explanations.
    """
    return prompt
...
def question_revise_simple(rows, filename, output_dir, revised_newpaper_folder, max_iterations=5, model='qwen3-235b-a22b-thinking-2507', temperature=0.6):
    ...
    """
    """
    llm = ChatOpenAI(temperature=temperature, model=model, api_key=os.getenv("DASHSCOPE_API_KEY"),base_url="https://dashscope.aliyuncs.com/compatible-mode/v1", max_tokens=2048, max_retries=5, extra_body={"thinking_budget": 1000})
    prompt_revise = ChatPromptTemplate.from_messages(
        [
            ("human", 
            '''
            You are an experienced Japanese N4/N5 examiner. There is a Japanese multiple-choice test at the end of this message. Some {errors} issues are found in those questions, each error type is provided with the question numbers that potentially have the issues. You should pay more attention to those questions.
    ...
    output_doc = Document()
    sentences = split_into_sentences(rows)
    for sentence in sentences:
        output_doc.add_paragraph(sentence)
    output_doc.save(output_path)
    
    qa_list = parse_questions(rows)
