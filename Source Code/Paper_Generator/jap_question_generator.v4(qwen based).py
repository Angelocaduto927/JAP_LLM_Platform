        ...
        if json_match:
            ...
            if question_numbers and comments:
                return question_numbers.group(1), comments.group(1)
                    ...
                    print(f"Warning: Missing JSON fields - question_numbers: {question_numbers is not None}, comments: {comments is not None}")
                    return "False", "[]"
        else:
            print("Warning: No JSON structure found in text")
            return "False", "[]"
    except Exception as e:
...
def has_multiple_correct_answers(text, llm, comments, quesiton_list):
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("human", 
            """
            **Here are the questions you need to review**:\n
            {input_data}\n\n
            Here are the comments you made for these questions in the last iteration, you should consider them when making your judgement for this round:\n
            {comments}\n\n
            The comments are one to one corresponding to the questions, and they are in the same order as the questions, eg. the first string in the list is the comment for Q1: もんだい1 and so on.\n
            You need to notice that some questions you judged as wrong questions are already modified. So for these questions: {question_list}, you don't need to consider the comments but need to pay more attention on them.\n
            ...
            **Format requirement**:
            ...
            3. Do not include any other comments.
            """
            ),
        ]
    )
...
def has_stem_errors(text, llm, comments, question_list):
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system", "You are an experienced Japanese N4/N5 examiner reviewing the following multiple-choice questions:\n\n"),
            ("human", 
...
def extract_wrong_questions(question_list_input, error, comments):
    ...
    comments_for_revise = [['Multiple correct answers problems'], ['Stem error problems'], ['Multiple correct answers and Stem error problems']]
...
def question_revise_simple(rows, filename, output_dir, revised_newpaper_folder, max_iterations=5, model='qwen3-235b-a22b-thinking-2507', temperature=0.6):
    ...
    prompt_revise = ChatPromptTemplate.from_messages(
        [
            ...
            {comments}
            The comments are one to one corresponding to the questions, and they are in the same order as the questions, eg. the first string in the list is the comment for the first question in the question list and so on.
            '''
            ),
        ]
    )
    ...
    for iteration in range(max_iterations):
        ...
        # 提取判断为错误的题目，进行下一轮修订
        wrong_questions, comments_for_revise = extract_wrong_questions(question_list, errors, comments)
