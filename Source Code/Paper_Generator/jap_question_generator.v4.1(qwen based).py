...
from jap_voting_machine import get_model_votes, aggregate_errors
...
def question_revise_simple(rows, filename, output_dir, revised_newpaper_folder, max_iterations=5, temperature=0.6):
    """
    ...
    revision_model = models[0]
    extra_body = {
        "enable_thinking": True,
        "thinking_budget": 1000
    } if revision_model.get("has_thinking", False) else {"enable_thinking": False}
    
    llm_revise = ChatOpenAI(
        temperature=temperature,
        model=revision_model["model_name"],
        api_key=os.getenv("DASHSCOPE_API_KEY"),
        base_url="https://dashscope.aliyuncs.com/compatible-mode/v1",
        max_tokens=2048,
        max_retries=5,
    ...
    for iteration in range(max_iterations):
        has_error, votes, comments = get_model_votes(revised_result, models, VOTE_THRESHOLD)
        if not has_error:
            print(f"No issues found after {iteration + 1} iterations.")
            print(f"Model votes: {votes}")
            break
        # 聚合多个模型检测到的错误
        consensus_errors = aggregate_errors(comments)
        
        print(f"Iteration {iteration + 1}")
        print(f"Model votes: {votes}")
        print(f"Consensus errors: {consensus_errors}")
        
        if not consensus_errors:
            print("No consensus on errors. Stopping iteration.")
            break
        
        # 提取判断为错误的题目，进行下一轮修订
        wrong_questions = extract_wrong_questions(question_list, consensus_errors)
        ...
        params['errors'] = consensus_errors
        ...
        with open(log_path, 'a', encoding='utf-8') as log_file:
            log_file.write(f"Iteration {iteration + 1} Errors: {consensus_errors}\n")
    else:
